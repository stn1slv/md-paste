...
- [ ] T001 Add `golang.org/x/net/html` dependency via `go get golang.org/x/net/html && go mod tidy`

## Phase 2: Foundational

Core data structures and GFM rendering logic that all user stories depend on.

- [ ] T002 Define `Table`, `Row`, `Cell` structs and `Alignment` enum in `internal/models/models.go`
- [ ] T003 Implement GFM table rendering logic in `internal/converter/gfm_renderer.go`
- [ ] T004 [P] Create unit tests for GFM rendering in `internal/converter/gfm_renderer_test.go`
...
- [ ] T005 [US1] Implement HTML table extraction logic using `golang.org/x/net/html` in `internal/converter/html_table.go`
- [ ] T006 [US1] Implement cell alignment extraction from `style` or `align` attributes in `internal/converter/html_table.go`
- [ ] T007 [US1] Implement Confluence macro stripping (extracting inner text) in `internal/converter/html_table.go`
- [ ] T008 [P] [US1] Create unit tests with HTML table samples in `internal/converter/html_table_test.go`
- [ ] T009 [US1] Integrate HTML table parsing into `internal/converter/converter.go`'s `Convert` function
...
- [ ] T010 [US2] Implement layout-aware text parser using whitespace heuristics in `internal/converter/text_table.go`
- [ ] T011 [P] [US2] Create unit tests for text table reconstruction in `internal/converter/text_table_test.go`
- [ ] T012 [US2] Integrate text table parsing as a fallback in `internal/converter/converter.go`
...
- [ ] T013 [US3] Update `Cell` model to include `RowSpan` and `ColSpan` in `internal/models/models.go`
- [ ] T014 [US3] Implement table grid flattening logic (injecting duplicate cells) in `internal/converter/flattening.go`
- [ ] T015 [P] [US3] Create unit tests for table flattening in `internal/converter/flattening_test.go`
- [ ] T016 [US3] Update `html_table.go` to populate `RowSpan` and `ColSpan` data during parsing

## Phase 6: Polish & Cross-Cutting Concerns

Ensuring robustness and consistency across the entire feature.

- [ ] T017 Ensure graceful fallback to standard `html-to-markdown` conversion if no table is detected in `internal/converter/converter.go`
- [ ] T018 Run `make lint` and `make format` to ensure code quality across all new files
- [ ] T019 Update `internal/cli/root_test.go` to include a table conversion integration test case
